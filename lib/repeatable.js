...
module.exports = function(Queue) {
  Queue.prototype.nextRepeatableJob = function(
    ...
    repeat.every = repeat.every * repeat.backoff;
    if (!_.isUndefined(repeat.max) && repeat.every > repeat.max) {
      if(repeat.cancelAtMax){
        return Promise.resolve();
      }
      repeat.every = repeat.max;    
    }
  ...
  function getRepeatKey(name, repeat, jobId) {
    ...
    // we only care about uniqueness on job id
